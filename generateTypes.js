...
function cleanupTypeNames(content) {
	return (
		content
			...
			.replace(
				/public([A-Z][a-zA-Z]*)(Row|Insert|Update|Relationships)?Schema/g,
				'$1',
			)
...
function prefixTypesWithT(content) {
	return (
		content
			// Add T prefix to type declarations if they don't already start with T
			.replace(/\btype\s+(?!T)([A-Z][a-zA-Z]*)/g, 'type T$1')
			// Add T prefix to interface declarations if they don't already start with T
			.replace(/\binterface\s+(?!T)([A-Z][a-zA-Z]*)/g, 'interface T$1')
	)
}
...
exec(supabaseCommand, (error, stdout, stderr) => {
	...
	exec(supazodCommand, (error, stdout, stderr) => {
		...
		// Clean up schemas.d.ts with additional type prefixing
		...
		const prefixedSchemasDtsContent = prefixTypesWithT(
			fixedSchemasDtsContent,
		)
		fs.writeFileSync(schemasDtsPath, prefixedSchemasDtsContent)
